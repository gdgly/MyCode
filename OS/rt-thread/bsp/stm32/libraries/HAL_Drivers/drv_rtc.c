...
static rt_err_t set_rtc_wakeup_timer(int wakeup_seconds)
{
#if defined(BSP_RTC_USING_WAKEUP_TIMER)
    /** Enable the WakeUp */
    if (HAL_RTCEx_SetWakeUpTimer_IT(&RTC_Handler, (wakeup_seconds - 1), RTC_WAKEUPCLOCK_CK_SPRE_16BITS) != HAL_OK)
    {
        return -RT_ERROR;
    }
    return RT_EOK;
#else
    LOG_E("RTC Wakeup timer hasn't been configured, please checkout rtconfig.h \"BSP_RTC_USING_WAKEUP_TIMER\" has defined.");
    return -RT_ERROR;
#endif
}
...
static rt_err_t rt_rtc_control(rt_device_t dev, int cmd, void *args)
{
    ...
    case RT_DEVICE_CTRL_RTC_SET_TIME:
        ...
        break;
    case RT_DEVICE_CTRL_RTC_SET_WAKEUP_SECONDS:
        if (set_rtc_wakeup_timer(*(rt_uint32_t *)args))
        {
            result = -RT_ERROR;
        }
        LOG_D("RTC: set rtc_wakeup seconds %d\n", *(rt_uint32_t *)args);
        break;
    case RT_DEVICE_CTRL_RTC_CLR_WAKEUP:
...
int rt_hw_rtc_init(void)
{
    ...
    int defaulttime = 1;
    /** Enable the WakeUp */
    rt_rtc_control(&rtc, RT_DEVICE_CTRL_RTC_SET_WAKEUP_SECONDS, &defaulttime);
#endif
