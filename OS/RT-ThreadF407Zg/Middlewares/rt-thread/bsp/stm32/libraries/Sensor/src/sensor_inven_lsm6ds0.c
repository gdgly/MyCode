...
#define DBG_TAG "sensor.inven.lsm6ds0"
#define DBG_LVL DBG_INFO
#include <rtdbg.h>
#include "drv_spi.h"

static struct lsm6ds0_device *mpu_dev;

static rt_err_t _lsm6ds0_init(struct rt_sensor_intf *intf)
{
    rt_uint8_t  i2c_addr = (rt_uint32_t)(intf->user_data) & 0xff;

    mpu_dev = lsm6ds0_init(intf->dev_name, i2c_addr);
...
static rt_err_t _lsm6ds0_set_range(rt_sensor_t sensor, rt_int32_t range)
{
    if (sensor->info.type == RT_SENSOR_CLASS_ACCE)
    {
        ...
        if (range < 2000)
            range_ctr = LSM6DS0_ACCEL_RANGE_2G;
        else if (range < 4000)
            range_ctr = LSM6DS0_ACCEL_RANGE_4G;
        else if (range < 8000)
            range_ctr = LSM6DS0_ACCEL_RANGE_8G;
        else
            range_ctr = LSM6DS0_ACCEL_RANGE_16G;

        LOG_D("acce set range %d", range_ctr);

        return lsm6ds0_set_param(mpu_dev, LSM6DS0_ACCEL_RANGE, range_ctr);
    }
    else if (sensor->info.type == RT_SENSOR_CLASS_GYRO)
    {
        ...
        if (range < 250000UL)
            range_ctr = LSM6DS0_GYRO_RANGE_250DPS;
        else if (range < 500000UL)
            range_ctr = LSM6DS0_GYRO_RANGE_500DPS;
        else if (range < 1000000UL)
            range_ctr = LSM6DS0_GYRO_RANGE_1000DPS;
        else
            range_ctr = LSM6DS0_GYRO_RANGE_2000DPS;

        LOG_D("gyro set range %d", range);

        return lsm6ds0_set_param(mpu_dev, LSM6DS0_GYRO_RANGE, range_ctr);
    }
    return RT_EOK;
}

static rt_err_t _lsm6ds0_acc_set_mode(rt_sensor_t sensor, rt_uint8_t mode)
{
...
static rt_err_t _lsm6ds0_set_power(rt_sensor_t sensor, rt_uint8_t power)
{
    ...
    if (power == RT_SENSOR_POWER_DOWN)
    {
        ...
        if (ref_count == 0)
        {
			...
			return 0;
            //return mpu6xxx_set_param(mpu_dev, MPU6XXX_SLEEP, LSM6DS0_SLEEP_ENABLE);
        }
        return RT_EOK;
    }
    else if (power == RT_SENSOR_POWER_NORMAL)
    {
		...
		return 0;
        //return mpu6xxx_set_param(mpu_dev, MPU6XXX_SLEEP, LSM6DS0_SLEEP_DISABLE);
    }
    else
    {
...
static rt_size_t _lsm6ds0_polling_get_data(rt_sensor_t sensor, struct rt_sensor_data *data)
{
    if (sensor->info.type == RT_SENSOR_CLASS_ACCE)
    {
        struct mpu6xxx_3axes acce;
        if (lsm6ds0_get_accel(mpu_dev, &acce) != RT_EOK)
        {
            return 0;
        }
    ...
    else if (sensor->info.type == RT_SENSOR_CLASS_GYRO)
    {
        ...
        if (lsm6ds0_get_gyro(mpu_dev, &gyro) != RT_EOK)
        {
            return 0;
        }
...
static rt_size_t lsm6ds0_fetch_data(struct rt_sensor_device *sensor, void *buf, rt_size_t len)
{
    RT_ASSERT(buf);

    if (sensor->config.mode == RT_SENSOR_MODE_POLLING)
    {
        return _lsm6ds0_polling_get_data(sensor, buf);
    }
    else
        return 0;
}

static rt_err_t lsm6ds0_control(struct rt_sensor_device *sensor, int cmd, void *args)
{
    ...
    case RT_SENSOR_CTRL_SET_RANGE:
        result = _lsm6ds0_set_range(sensor, (rt_int32_t)args);
        break;
    case RT_SENSOR_CTRL_SET_ODR:
        result = -RT_EINVAL;
        break;
    case RT_SENSOR_CTRL_SET_MODE:
        result = _lsm6ds0_acc_set_mode(sensor, (rt_uint32_t)args & 0xff);
        break;
    case RT_SENSOR_CTRL_SET_POWER:
        result = _lsm6ds0_set_power(sensor, (rt_uint32_t)args & 0xff);
        break;
...
int rt_hw_lsm6ds0_init(const char *name, struct rt_sensor_config *cfg)
{
    rt_int8_t result;
    rt_sensor_t sensor_acce = RT_NULL, sensor_gyro = RT_NULL;

#ifdef PKG_USING_LSM6DS0_ACCE
    /* accelerometer sensor register */
    {
        ...
        sensor_acce->info.model      = "lsm6ds_acc";
    ...
    /* gyroscope sensor register */
    {
        ...
        sensor_gyro->info.model      = "lsm6ds_gyro";
