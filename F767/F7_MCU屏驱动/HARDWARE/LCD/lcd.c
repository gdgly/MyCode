...
//	
	...
	//FSMC读时序控制寄存器
    FSMC_ReadWriteTim.AddressSetupTime=0x40;    	//地址建立时间(ADDSET)为15个HCLK 1/216M=4.6ns*15=69ns
    FSMC_ReadWriteTim.AddressHoldTime=0x40;
    FSMC_ReadWriteTim.DataSetupTime=0x40;       	//数据保存时间(DATAST)为85个HCLK	=4.6*80=368ns
    FSMC_ReadWriteTim.AccessMode=FMC_ACCESS_MODE_A; //模式A
    //FSMC写时序控制寄存器
    FSMC_WriteTim.AddressSetupTime=0x40;        	//地址建立时间(ADDSET)为15个HCLK=69ns
    FSMC_WriteTim.AddressHoldTime=0x40;
    FSMC_WriteTim.DataSetupTime=0x40;           	//数据保存时间(DATAST)为4.6ns*15个HCLK=69ns
    FSMC_WriteTim.AccessMode=FMC_ACCESS_MODE_A;	//模式A
	HAL_SRAM_Init(&SRAM_Handler,&FSMC_ReadWriteTim,&FSMC_WriteTim);		
