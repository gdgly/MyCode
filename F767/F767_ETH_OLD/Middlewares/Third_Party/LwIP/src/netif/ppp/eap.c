...
u_char *str;
{
    ...
    msglen = EAP_HEADERLEN + 2 * sizeof (u_char) + sizeof (uint32_t_t) +
             SHA_DIGESTSIZE;
    p = pbuf_alloc(PBUF_RAW, (u16_t)(PPP_HDRLEN + msglen), PPP_CTRL_PBUF_TYPE);
    if(NULL == p)
        return;
...
static void eap_request(ppp_pcb *pcb, u_char *inp, int id, int len)
{
    ...
    case EAPT_SRP:
        ...
        case EAPSRP_SVALIDATOR:
            ...
            {
                len -= sizeof (uint32_t_t) + SHA_DIGESTSIZE;
...
static void eap_response(ppp_pcb *pcb, u_char *inp, int id, int len)
{
    ...
    case EAPT_SRP:
        ...
        case EAPSRP_CVALIDATOR:
            ...
            if (len < sizeof (uint32_t_t) + SHA_DIGESTSIZE)
            {
                ppp_error("EAP: M1 length %d < %d", len,
                          sizeof (uint32_t_t) + SHA_DIGESTSIZE);
                eap_figure_next_state(pcb, 1);
                break;
            }
...
static int eap_printpkt(const u_char *inp, int inlen, void (*printer) (void *, const char *, ...), void *arg)
{
    ...
    case EAP_REQUEST:
        ...
        case EAPT_SRP:
            ...
            case EAPSRP_SVALIDATOR:
                if (len < (int)sizeof (uint32_t_t))
                    break;
                GETLONG(uval, inp);
                len -= sizeof (uint32_t_t);
                if (uval & SRPVAL_EBIT)
                {
                    printer(arg, " E");
                    uval &= ~SRPVAL_EBIT;
