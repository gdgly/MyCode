...
void parse_data(PARSE_STRUCT *parse_s, uint8_t *receive_buffer, uint16_t receive_len)
{
    ...
    for (; data_cnt < receive_len; data_cnt++)
    {
        ...
        //GETHEAD
        case GETHEAD:
#ifdef SUPPORT_TO_ONE
            if (receive_buffer[data_cnt] == OLD_PROTOCOL_VER)
            {
                parse_s->sta = GETVERSION;
                parse_s->version = OLD_PROTOCOL_VER;
            }
            else
#endif
            
#ifdef SUPPORT_TO_TWO
			if (receive_buffer[data_cnt] == NEW_PROTOCOL_VER)
            {
                parse_s->sta = GETVERSION;
                parse_s->version = NEW_PROTOCOL_VER;
            }
            else
#endif
        ...
        //GETVERSION
        case GETVERSION:
#if defined(SUPPORT_TO_ONE)
            if (parse_s->version == OLD_PROTOCOL_VER)
            {
...
#ifdef SUPPORT_TO_TWO
			if (parse_s->version == NEW_PROTOCOL_VER)
            {
                parse_s->frame_data_len = receive_buffer[data_cnt];
        ...
        case GETLENHALF:
            ...
            if (parse_s->version == NEW_PROTOCOL_VER)
            {
                ...
                else
                {
                    ...
                    {
                        *(parse_s->frame_buffer + 0) = VERSION_HEAD;
                        *(parse_s->frame_buffer + 1) = NEW_PROTOCOL_VER;
        ...
        case GETLEN:
            ...
            if (parse_s->version == OLD_PROTOCOL_VER)
            {
                ...
                *(parse_s->frame_buffer + 3 + parse_s->frame_cnt) = receive_buffer[data_cnt];
                ...
                if (parse_s->frame_cnt >= (parse_s->frame_data_len + 6))
                {

                    if (check_frame(parse_s, parse_s->frame_buffer + 2, (parse_s->frame_data_len + 7)) == 0)
                    {

                        parse_s->frame_len = parse_s->frame_data_len + 9;
