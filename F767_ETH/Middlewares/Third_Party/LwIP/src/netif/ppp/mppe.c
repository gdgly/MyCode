...
static void mppe_rekey(ppp_mppe_state * state, int initial_key)
{
    ...
    uint8_t_t sha1_digest[SHA1_SIGNATURE_SIZE];
 ...
 */
void
mppe_init(ppp_pcb *pcb, ppp_mppe_state *state, uint8_t_t options)
{
#if PPP_DEBUG
    const uint8_t_t *debugstr = (const uint8_t_t*)"mppe_comp_init";
    if (&pcb->mppe_decomp == state)
    {
        debugstr = (const uint8_t_t*)"mppe_decomp_init";
    }
#endif /* PPP_DEBUG */
...
mppe_compress(ppp_pcb *pcb, ppp_mppe_state *state, struct pbuf **pb, u16_t protocol)
{
    ...
    *pb = np;
    ...
    {
        lwip_arc4_crypt(&state->arc4, (uint8_t_t*)n->payload, n->len);
        if (n->tot_len == n->len)
        {
            break;
        }
