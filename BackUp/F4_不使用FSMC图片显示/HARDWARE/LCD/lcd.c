...
//	GPIOD->ODR = 0X0000;     //全部输出0
    ...
    GPIO_InitStructure.GPIO_Pin = (3<<0)|(7<<8)|(3<<14);//PD0,1,8,9,10,14,15 AF OUT
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IN;//复用输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;//上拉
    GPIO_Init(GPIOD, &GPIO_InitStructure);//初始化
...
//	FSMC_NORSRAMTimingInitTypeDef  writeTiming;
    ...
    GPIO_InitStructure.GPIO_Pin = GPIO_Pin_12;		//RS
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;	//复用输出
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;	//推挽输出
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_100MHz;//100MHz
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;	//上拉
...
void LCD_Dispy_Pic(const unsigned char *buff)
{
    ...
    for(i=240; i<480; i++)
    {
        for(j=120; j<360; j++)
        {
			...
			LCD_WriteRAM_Prepare();		//开始写入GRAM
			temp = (u16)(buff[k+1]<<8)|buff[k];
			LCD_WR_DATA(temp);			//设置光标位置
			k += 2;
		}
    }
}
