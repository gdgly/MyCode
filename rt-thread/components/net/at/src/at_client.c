...
int at_obj_set_resp_start(at_client_t client, at_response_t resp)
{
    rt_err_t result = RT_EOK;
...
int at_obj_set_resp_end(at_client_t client, at_response_t resp)
{
    rt_err_t result = RT_EOK;
    ...
    if (resp != RT_NULL)
    {
        if (rt_sem_take(client->resp_notice, resp->timeout) != RT_EOK)
        {
            LOG_D("wait resp timeout (%d ticks)!", resp->timeout);
            client->resp_status = AT_RESP_TIMEOUT;
            result = -RT_ETIMEOUT;
            goto __exit;
        }
...
#define AT_CLIENT_THREAD_STACK         768
    ...
    {
        LOG_E("AT client initialize failed! at_client_lock create failed!");
        result = -RT_ENOMEM;
        goto __exit;
    }
